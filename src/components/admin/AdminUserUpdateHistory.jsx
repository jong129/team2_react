...
const AdminUserUpdateHistory = () => {
  ...
  const fetchList = async () => {
    ...
    setError("");
    try {
      const res = await axiosInstance.get("/api/admin/member-update-histories", {
        params: {
          keyword: query || undefined,
          fieldName: fieldNameQuery || undefined,
  ...
  }, [query, fieldNameQuery, changeTypeQuery, page, size, from, to]);

  const onSearch = () => {
    setPage(0);
    setQuery(keyword.trim());
    setFieldNameQuery(fieldName);
  ...
  const purgeByPeriod = async () => {
    ...
    if (!window.confirm(`${from} ~ ${to} 기간의 회원정보 수정 이력을 삭제할까요?`)) return;

    try {
      await axiosInstance.post("/api/admin/member-update-histories/purge", null, {
        params: { from, to },
      });
      alert("기간 삭제 완료");
      setPage(0);
    ...
    return (
      <div className="container py-5" style={{ fontFamily: "'Pretendard', sans-serif" }}>
        <div className="d-flex justify-content-between align-items-center mb-3">
          <h2 className="fw-bold m-0">회원정보 수정 이력</h2>
            ...
            <table className="table mb-0 align-middle">
              <thead className="table-light">
                <tr>
                  ...
                  <th style={{ width: 170 }}>대상ID</th>
                  <th style={{ width: 140 }}>대상이름</th>
                  <th style={{ width: 170 }}>수정자ID</th>
                  <th style={{ width: 220 }}>시각</th>
                  <th style={{ width: 130 }}>FIELD</th>
                  <th style={{ width: 140 }}>TYPE</th>
                  <th style={{ minWidth: 220 }}>OLD</th>
                  <th style={{ minWidth: 220 }}>NEW</th>
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  ...
                  rows.map((r, idx) => (
                    <tr key={r.historyId ?? `${r.memberId}-${r.changedAt}-${idx}`}>
                      ...
                      <td>{r.fieldName}</td>
                      <td>{r.changeType}</td>
                      <td className="text-truncate" style={{ maxWidth: 260 }}>{r.oldValue || "-"}</td>
                      <td className="text-truncate" style={{ maxWidth: 260 }}>{r.newValue || "-"}</td>
                    </tr>
                  ))
                )}
              </tbody>
...
export default AdminUserUpdateHistory;
