...
const AdminActivityHistory = () => {
  ...
  const [actionTypeQuery, setActionTypeQuery] = useState("");
  ...
  const fetchList = async () => {
    ...
    setError("");
    try {
      const res = await axiosInstance.get("/api/admin/activity-logs", {
        params: {
          keyword: query || undefined,
          actionType: actionTypeQuery || undefined,
          page,
          size,
          from: from || undefined,
          to: to || undefined,
  ...
  }, [query, actionTypeQuery, page, size, from, to]);

  const onSearch = () => {
    setPage(0);
    setQuery(keyword.trim());
    setActionTypeQuery(actionType);
  };
  ...
  const purgeByPeriod = async () => {
    ...
    const label = actionTypeQuery ? `actionType=${actionTypeQuery}` : "전체";
    if (!window.confirm(`${from} ~ ${to} 기간의 활동 로그(${label})를 삭제할까요?`)) return;

    try {
      await axiosInstance.post("/api/admin/activity-logs/purge", null, {
        params: {
          from,
          to,
          actionType: actionTypeQuery || undefined,
          ...
          <div className="row g-2 align-items-center">
            <div className="col-md-5">
              <input
                ...
                placeholder="검색 (아이디 / 이름 / 타입 / 상세 / IP)"
                value={keyword}
            ...
            <div className="col-md-3">
              ...
              >
                <option value="">ACTION_TYPE 전체</option>
            ...
            <table className="table mb-0 align-middle">
              <thead className="table-light">
                <tr>
                  ...
                  <th style={{ width: 170 }}>로그인ID</th>
                  <th style={{ width: 140 }}>이름</th>
                  <th style={{ width: 170 }}>ACTION_TYPE</th>
                  <th>상세</th>
              ...
              <tbody>
                {loading ? (
                  ...
                  rows.map((r, idx) => (
                    <tr key={r.logId ?? `${r.memberId}-${r.actionAt}-${idx}`}>
